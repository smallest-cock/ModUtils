...
namespace PluginUpdates {
	...
	*/
	void checkForUpdates(const std::string &modName, const std::string &currentVersion, const std::string &assetName) {
		...
		HttpWrapper::SendCurlRequest(req, [modName, currentVersion, assetName](int code, std::string result) {
			if (code != 200) {
	...
	void downloadAndInstallUpdaterPlugin(std::shared_ptr<GameWrapper> gw, const std::string &updateCmd) {
		...
		HttpWrapper::SendCurlRequest(req, [pluginsFolder, configFile, downloadFile, updateCmd](int code, std::string result) {
			try {
				if (code != 200) {
				...
				req.verb = "GET";
				req.url  = *urlOpt;
				HttpWrapper::SendCurlRequest(
				    req, downloadFile.wstring(), [pluginsFolder, configFile, updateCmd](int httpCode, std::wstring filePath) {
					    if (httpCode != 200) {
...
namespace Colors {
	...
	std::optional<FColor> hexToFColor(const std::string &hex) {
		if (hex.size() != 8) {
			LOG("ERROR: Color hex string \"{}\" isn't 8 characters. Falling back to white...");
			return FColor{255, 255, 255, 255}; // fallback to white
		}
